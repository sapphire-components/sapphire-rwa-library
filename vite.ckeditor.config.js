...
const OUT_FILE = 'sapphire-rwa-ckeditor.js';

function makeBanner(mode) {
	return `/*!  SapphireRWACKEditor ${mode.toUpperCase()} v${pkg.version} ${new Date().toISOString()} */\n`;
}
...
function bannerOnDisk(banner) {
	return {
		...
		writeBundle(options) {
			...
			if (content.startsWith('/*!  SapphireRWACKEditor')) return;
...
// CKEditor plugins import CSS. Lib-mode Vite would emit a sibling .css file;
// fold it into the IIFE so screens that skip this script also skip the styles.
function inlineCssIntoJs() {
	return {
		...
		generateBundle(_options, bundle) {
			...
			const inject =
				`(function(){if(document.getElementById('sapphire-rwa-ckeditor-extra-css'))return;var s=document.createElement('style');s.id='sapphire-rwa-ckeditor-extra-css';s.textContent=${JSON.stringify(css)};document.head.appendChild(s);})();\n`;
