...
| ---------- | ---------------------------------------------------------------------------------------------------- | --------------------------------------- |
| `Change`   | Fired when the committed value changes (after debounce while typing, or immediately on blur / step). | `Identifier` (`Text`), `Value` (`Text`) |
| `Clear`    | Fired when the clear control is activated (click or `Enter`).                                        | `Identifier` (`Text`)                   |
| `EnterKey` | Fired when `Enter` is pressed, after any pending change has been flushed.                            | `Identifier` (`Text`), `Value` (`Text`) |
