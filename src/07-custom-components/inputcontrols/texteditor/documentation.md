...
| ---------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `Content`        | `Text`    | HTML content of the editor. Applied on init only. To replace content after init, call `SetTextEditorContent`.                               |
...
| `Change` | Fired on every user edit (Quill `text-change`). Empty editor → `""`. Does not fire for `SetTextEditorContent`. | `Identifier` (`Text`), `Text` (`Text`), `Html` (`Text`) |
...
| Name                   | Description                                                                                                                                                                                                                  | Arguments                                 |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| `SetTextEditorContent` | Global client action to replace the editor HTML from outside (instead of changing the `Content` input). Sanitized with DOMPurify and applied silently (does not fire `Change`). `Identifier` must match the widget instance. | `Identifier` (`Text`), `Content` (`Text`) |
