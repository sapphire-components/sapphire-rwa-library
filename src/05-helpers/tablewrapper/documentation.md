...
- When `ReorderOnDrop` is `False`, reorder is staged and `Reorder` does **not** fire on drag or on typed position changes:
  - Each body row's first cell shows the staged position in normal flow (it uses that cell's padding and margins): origin, arrow, then the intended position (`1 → 1`, `4 → 2`, …). Whatever was already in the cell — an empty column, or other content — is hidden for the staged session and restored when `ReorderableRows` or `ReorderOnDrop` leaves this mode. Example: dragging the 4th row two places up yields `1 → 1`, `4 → 2`, `2 → 3`, `3 → 4`, `5 → 5`. Further drags keep those original numbers on the left and only update the intended side.
  ...
  - A bar above the table shows **Apply order**. It is disabled until the visual order differs from the session origin, and while `IsLoading` is `True`. Clicking it fires `Reorder` with the current `tr` `data-rowid` list, then treats that order as the new origin (cells return to `1 → 1`, `2 → 2`, … and the button disables again).
...
| `ReorderOnDrop`     | `Boolean`         | When `True` (default), a drop that changes order fires `Reorder` immediately. When `False`, drag and typed position changes only rearrange the DOM: each row's first cell shows original → intended (`origin →` editable index), replacing that cell's content until staged reorder is turned off. Enter moves the row within `1…N` (invalid / `Escape` reverts). **Apply order** is the only commit; it is disabled until the order actually changed. |
