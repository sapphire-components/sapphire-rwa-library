...
| Name          | Description                                                                                                                         |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `Change`      | Fired when the selection changes. Payload is a JSON string of the full `SelectedList` array (always an array, including single select). |
| `Clear`       | Fired when the trigger clear control clears the entire selection (also followed by `Change`).                                       |
| `ScrollEnded` | Fired when the options list is scrolled to (near) the bottom; resets when the user scrolls away from the end.                       |
| `SearchClear` | Fired when the search field is cleared.                                                                                             |
| `Search`      | Fired (debounced) with the current keyword while searching; primarily used with `SearchServerSide`.                                 |
