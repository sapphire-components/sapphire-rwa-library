...
- `Change` fires only when the selection reaches `SelectableCount`. The payload is a JSON string of the selected ISO day integers (for example `"[1,3,5]"`), in selection order.
...
| `Selected`        | `Text`    | Currently selected ISO weekdays. Accepts a JSON array string such as `"[1,3,5]"` (same shape as the `Change` payload), or an equivalent list of integers `1`–`7`. Values outside that range are ignored. |
...
| Name     | Description                                                                                                                                                          |
| -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Change` | Fired when the user reaches exactly `SelectableCount` selected days. Receives a JSON string of the selected ISO day integers (e.g. `"[2,4,6]"`), in selection order. |
